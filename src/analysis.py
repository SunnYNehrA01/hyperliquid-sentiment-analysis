...
def sentiment_comparison(df):
    ...
    for metric in metrics:
        ...
        if len(fear_vals) > 20 and len(greed_vals) > 20:
            ...
            tests[metric] = {"statistic": float(stat), "p_value": float(pvalue)}
        else:
            tests[metric] = {"statistic": np.nan, "p_value": np.nan}

    return summary, pd.DataFrame(tests).T.reset_index().rename(columns={"index": "metric"})
